...
- **Optional terminal operations view** - the first `fm-tui` slice provides a persistent Firstmate hub, active managed and private worker destinations, a current-state inspector, durable Reports, bounded Live output, deterministic snapshots, authority-aware messaging, and safe private Codex thread creation.
