...
- **Optional terminal operations view** - the first `fm-tui` slice provides a persistent Firstmate hub with bounded conversation history, active managed and private worker destinations, a current-state inspector, durable Reports, bounded Live output, deterministic snapshots, authority-aware messaging, and safe private Codex thread creation.
...
- [docs/tui.md](docs/tui.md) - run the optional terminal hub and worker view, send through existing authorities, create private Codex threads, and print deterministic snapshots.
