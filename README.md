...
- **Optional terminal worker view** - the first `fm-tui` slice provides active-worker filtering, explicit Firstmate-managed and captain-private ownership groups, a current-state inspector, durable Reports, bounded Live output, deterministic snapshots, and a selected-worker message composer routed through the existing send authorities.
...
- [docs/tui.md](docs/tui.md) - run the optional read-only task TUI and its deterministic snapshot.
